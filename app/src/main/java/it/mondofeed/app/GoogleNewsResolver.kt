...
object GoogleNewsResolver {
    ...
    fun isGoogleNewsUrl(url: String): Boolean =
        runCatching { HttpUrl.get(url).host.endsWith("news.google.com") }
            .getOrDefault(false)

    private fun extractQueryTarget(url: String): String? {
        val parsed = runCatching { HttpUrl.get(url) }.getOrNull() ?: return null
    ...
    private fun isPublisherUrl(url: String): Boolean {
        val parsed = runCatching { HttpUrl.get(url) }.getOrNull() ?: return false
